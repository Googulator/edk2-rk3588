...
#include <IndustryStandard/Rk358xVop2.h>
    ...
    )
{
    ...
    HAL_CRU_ClkSetFreq (PLL_GPLL, mCurrentTimings->FrequencyKHz * 1000);

    gBS->Stall (100000); // XXX wait for lock

    Rate = HAL_CRU_ClkGetFreq (PLL_GPLL);
    DEBUG ((DEBUG_INFO, "Vop2SetMode(): GPLL rate %u Hz\n", Rate));
